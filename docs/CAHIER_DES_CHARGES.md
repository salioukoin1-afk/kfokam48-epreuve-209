...
| Blocage après 5 échecs de code | Q4 : bloquer 2 minutes après 5 erreurs, sans préciser la granularité du blocage (par étudiant ? par IP ? par session ?) ni le comportement HTTP attendu pendant le blocage. Précision v1.3 : un échec de **code inconnu** ne permet pas d'identifier la session (c'est précisément la devinette visée par Q4) — le compteur des échecs de codes inconnus est donc **global à l'étudiant** (toutes sessions) ; une fois le code reconnu, les autres erreurs (expiration, clôture, doublon) ne sont pas des tentatives de devinette et n'alimentent pas le compteur | Pendant le blocage, toute nouvelle tentative est refusée `429 BLOCAGE_TENTATIVES` **avant même de chercher le code** (ordre testé de D3) | Le client craint que les étudiants « devinent les codes entre eux » (Q4) : bloquer par étudiant identifié empêche un même étudiant de multiplier les essais, sans pénaliser toute la salle sur une IP partagée ; le 429 est le statut sémantique standard et reste hors des codes imposés, donc libre |
...
| 1.3 | Pendant l'étape 2 (US-02) | RG3 précisée : le compteur des échecs de codes inconnus est global à l'étudiant (impossible d'identifier la session depuis un code inconnu — c'est la devinette visée par Q4) ; les autres erreurs n'alimentent pas le compteur ; le 429 est vérifié avant toute recherche de code. Migration V4 (session_id nullable dans tentative_code) |
